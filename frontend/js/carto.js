...
window.cartoState = {
  ...
  lens: 'demo',
  sub: {
    equipements: new Set(['ecoles', 'sante', 'commerces']),
    mobilite: new Set(['tc', 'velo', 'gares']),
  },
  buffer: { equipements: 0, mobilite: 0 },
...
const CARTO_LENSES = [
  { id: 'demo', n: 'Densité de population', d: 'Habitants par km²', c: 'var(--eau)', type: 'choro', metric: 'pop' },
  { id: 'seniors', n: 'Part des seniors', d: '60 ans et plus (%)', c: 'var(--ardoise)', type: 'choro', metric: 'seniors' },
  { id: 'jeunes', n: 'Part des jeunes', d: 'Moins de 15 ans (%)', c: 'var(--vegetal)', type: 'choro', metric: 'jeunes' },
  { id: 'revenu', n: 'Revenu médian', d: 'Revenu disponible médian', c: 'var(--soleil)', type: 'choro', metric: 'revenu' },
  { id: 'equipements', n: 'Équipements & accès', d: 'Écoles, santé, commerces', c: 'var(--soleil)', type: 'poi' },
  { id: 'mobilite', n: 'Mobilité', d: 'Transports collectifs & vélo', c: 'var(--vegetal)', type: 'poi' },
  { id: 'equipdens', n: 'Densité d\'équipements', d: 'Équipements BPE par km²', c: 'var(--soleil)', type: 'choro', metric: 'equip' },
  { id: 'commerces', n: 'Densité de commerces', d: 'Commerces par km²', c: 'var(--terre)', type: 'choro', metric: 'commerces' },
  { id: 'emissions', n: 'Émissions transport', d: 'GES routiers par habitant', c: 'var(--ardoise)', type: 'choro', metric: 'ges' },
  { id: 'ges_total', n: 'Émissions GES totales', d: 'tCO₂ par habitant', c: 'var(--terre)', type: 'choro', metric: 'ges_total' },
  { id: 'environnement', n: 'Artificialisation', d: 'Part de sols artificialisés', c: 'var(--terre)', type: 'choro', metric: 'artif' },
];
const CARTO_RAMP = {
...
const POICOLOR = { ecoles: 'var(--soleil)', sante: 'var(--red)', commerces: 'var(--terre)', tc: 'var(--vegetal)', gares: 'var(--eau)', velo: '#1f9d55' };
const POIHEX = { ecoles: '#d79a1c', sante: '#e2001a', commerces: '#b5642f', tc: '#3c8a5e', gares: '#16798c', velo: '#1f9d55' };
const POINAME = { ecoles: 'Écoles', sante: 'Santé', commerces: 'Commerces', tc: 'Arrêts TC', velo: 'Pistes cyclables', gares: 'Gares' };
...
function renderCtrl() {
  ...
  const opts = window.cartoState.lens === 'equipements'
    ? [['ecoles', 'Écoles'], ['sante', 'Santé'], ['commerces', 'Commerces']]
    : [['tc', 'Arrêts TC'], ['velo', 'Pistes cyclables'], ['gares', 'Gares']];
  ...
  ctrl.querySelectorAll('.chip').forEach(ch => ch.onclick = () => {
    ...
    drawPoi([...sub]).then(() => { drawBuffer(); renderCtrl(); updateInsight(); });
  });
...
async function applyLens() {
  ...
  const L0 = CARTO_LENSES.find(l => l.id === window.cartoState.lens);
  _clearChoro(); _clearPoi(); _clearBuffer(); _clearCommunesBase();

  drawCommunesBase();
  if (L0 && L0.type === 'choro') {
    await drawChoro(L0.metric);
  } else if (L0 && L0.type === 'poi') {
    await drawPoi([...window.cartoState.sub[L0.id]]);
    drawBuffer();
  }
